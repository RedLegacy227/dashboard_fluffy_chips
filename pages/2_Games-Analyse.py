...
try:
    ...
    try:
        ...
        def display_analysis(data, title, team_name, is_home):
            st.markdown(f"### 💹 {title} ###")
            ...
            if is_home:
                st.markdown(f"* {team_name} was Fav {home_fav} Times")
                st.markdown(f"* {team_name} was Equals {home_equals} Times")
                st.markdown(f"* {team_name} was not Fav {home_not_fav} Times")
            else:
                st.markdown(f"* {team_name} was Fav {away_fav} Times")
                st.markdown(f"* {team_name} was Equals {away_equals} Times")
                st.markdown(f"* {team_name} was not Fav {away_not_fav} Times")
            ...
            for column in ['FT_Odd_H', 'FT_Odd_D', 'FT_Odd_A']:
                ...
                if column == 'FT_Odd_H':
                    if is_home:
                        st.markdown(f"* {team_name} was Fav and Won {len(data[(data['FT_Odd_H'] < data['FT_Odd_A'] - 1.00) & (data['FT_Goals_H'] > data['FT_Goals_A'])])} Games")
                        st.markdown(f"* {team_name} was Equals and Won {len(data[(data['FT_Odd_H'] >= data['FT_Odd_A'] - 1.00) & (data['FT_Odd_H'] <= data['FT_Odd_A'] + 1.00) & (data['FT_Goals_H'] > data['FT_Goals_A'])])} Games")
                        st.markdown(f"* {team_name} was not Fav and Won {len(data[(data['FT_Odd_H'] > data['FT_Odd_A'] + 1.00) & (data['FT_Goals_H'] > data['FT_Goals_A'])])} Games")
                        st.markdown(f"* {team_name} was Fav and Lost {len(data[(data['FT_Odd_H'] < data['FT_Odd_A'] - 1.00) & (data['FT_Goals_H'] < data['FT_Goals_A'])])} Games")
                        st.markdown(f"* {team_name} was Equals and Lost {len(data[(data['FT_Odd_H'] >= data['FT_Odd_A'] - 1.00) & (data['FT_Odd_H'] <= data['FT_Odd_A'] + 1.00) & (data['FT_Goals_H'] < data['FT_Goals_A'])])} Games")
                        st.markdown(f"* {team_name} was not Fav and Lost {len(data[(data['FT_Odd_H'] > data['FT_Odd_A'] + 1.00) & (data['FT_Goals_H'] < data['FT_Goals_A'])])} Games")
                    else:
                        st.markdown(f"* {team_name} was Fav and Won {len(data[(data['FT_Odd_A'] < data['FT_Odd_H'] - 1.00) & (data['FT_Goals_A'] > data['FT_Goals_H'])])} Games")
                        st.markdown(f"* {team_name} was Equals and Won {len(data[(data['FT_Odd_A'] >= data['FT_Odd_H'] - 1.00) & (data['FT_Odd_A'] <= data['FT_Odd_H'] + 1.00) & (data['FT_Goals_A'] > data['FT_Goals_H'])])} Games")
                        st.markdown(f"* {team_name} was not Fav and Won {len(data[(data['FT_Odd_A'] > data['FT_Odd_H'] + 1.00) & (data['FT_Goals_A'] > data['FT_Goals_H'])])} Games")
                        st.markdown(f"* {team_name} was Fav and Lost {len(data[(data['FT_Odd_A'] < data['FT_Odd_H'] - 1.00) & (data['FT_Goals_A'] < data['FT_Goals_H'])])} Games")
                        st.markdown(f"* {team_name} was Equals and Lost {len(data[(data['FT_Odd_A'] >= data['FT_Odd_H'] - 1.00) & (data['FT_Odd_A'] <= data['FT_Odd_H'] + 1.00) & (data['FT_Goals_A'] < data['FT_Goals_H'])])} Games")
                        st.markdown(f"* {team_name} was not Fav and Lost {len(data[(data['FT_Odd_A'] > data['FT_Odd_H'] + 1.00) & (data['FT_Goals_A'] < data['FT_Goals_H'])])} Games")
    
                elif column == 'FT_Odd_D':
                    if is_home:
                        st.markdown(f"* {team_name} was Fav and Tied {len(data[(data['FT_Odd_H'] < data['FT_Odd_A'] - 1.00) & (data['FT_Goals_H'] == data['FT_Goals_A'])])} Games")
                        st.markdown(f"* {team_name} was Equals and Tied {len(data[(data['FT_Odd_H'] >= data['FT_Odd_A'] - 1.00) & (data['FT_Odd_H'] <= data['FT_Odd_A'] + 1.00) & (data['FT_Goals_H'] == data['FT_Goals_A'])])} Games")
                        st.markdown(f"* {team_name} was not Fav and Tied {len(data[(data['FT_Odd_H'] > data['FT_Odd_A'] + 1.00) & (data['FT_Goals_H'] == data['FT_Goals_A'])])} Games")
                    else:
                        st.markdown(f"* {team_name} was Fav and Tied {len(data[(data['FT_Odd_A'] < data['FT_Odd_H'] - 1.00) & (data['FT_Goals_A'] == data['FT_Goals_H'])])} Games")
                        st.markdown(f"* {team_name} was Equals and Tied {len(data[(data['FT_Odd_A'] >= data['FT_Odd_H'] - 1.00) & (data['FT_Odd_A'] <= data['FT_Odd_H'] + 1.00) & (data['FT_Goals_A'] == data['FT_Goals_H'])])} Games")
                        st.markdown(f"* {team_name} was not Fav and Tied {len(data[(data['FT_Odd_A'] > data['FT_Odd_H'] + 1.00) & (data['FT_Goals_A'] == data['FT_Goals_H'])])} Games")
    ...
    if not team_data.empty:
        ...
        stats_G_Attempts_pG_IF_home = team_data['Avg_Goal_Attempts_per_Goal_InFavor_H'].values[0]
        stats_G_Attempts_pG_Ag_home = team_data['Avg_Goal_Attempts_per_Goal_Against_H'].values[0]
        stats_G_Attempts_pG_IF_away = team_data['Avg_Goal_Attempts_per_Goal_InFavor_A'].values[0]
        stats_G_Attempts_pG_Ag_away = team_data['Avg_Goal_Attempts_per_Goal_Against_A'].values[0]
        stats_yellow_cards_home = team_data['Avg_Yellow_Cards_H'].values[0]
        ...
        with col1:
            ...
            st.markdown(f"🟨 Yellow Cards Taken ***{selected_away}*** ➡️ ***{stats_yellow_cards_away:.2f}***")
        with col2:
            st.divider()
            st.markdown(f"#### Average Stats on the Last 7 Games ####")
            st.divider()
            ...
            st.divider()   
            st.markdown(f"🟥 Red Cards Taken ***{selected_home}*** ➡️ ***{stats_red_cards_home:.2f}***")
